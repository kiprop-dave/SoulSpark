...
export default function BasicDetailsTab({ nextStep, prevStep, confirmStep }: BasicDetailsTabProps): JSX.Element {
    ...
    <div className="w-full">
      ...
      <form onSubmit={(e) => handleSubmit(e)}>
        <div className="flex flex-col items-start justify-start w-full mt-4">
          <label htmlFor="bio" className="text-sm font-bold text-slate-500">Bio</label>
          <textarea id="bio" className="w-full p-2 h-32 border border-red-200 outline-red-500 focus:ring-1 focus:ring-red-500 rounded font-sans text-sm"
        ...
        <div className="w-full mt-4">
          <label htmlFor="languages" className="text-sm font-bold text-slate-500">Languages</label>
          <div className="grid grid-cols-4 w-full mt-2 gap-2">
            {
              languages.map((lang, i) => {
                return <OptionSelect
                  ...
                  property="languages" selectOption={handleMultipleOptions}
                  isMulti
                />
              })
            }
        ...
        <div className="w-full mt-4">
          <label htmlFor="zodiac" className="text-sm font-bold text-slate-500">Zodiac</label>
          <div className="grid grid-cols-4 w-full mt-2 gap-2">
            {
              zodiacSigns.map((sign) => {
                return <OptionSelect key={sign} option={sign} isSelected={basicInfo?.zodiac === sign} selectOption={handleSingleOption} property="zodiac" />
              })
            }
          </div>
        </div>
        <div className="w-full mt-4">
          <label htmlFor="education" className="text-sm font-bold text-slate-500">What is your education level?</label>
          <div className="grid grid-cols-3 w-full mt-2 gap-2">
            {
              educationLevels.map((level) => {
                return <OptionSelect key={level} option={level} isSelected={basicInfo?.education === level} property="education" selectOption={handleSingleOption} />
              })
            }
          </div>
        </div>
        <div className="w-full mt-4">
          <label htmlFor="occupation" className="text-sm font-bold text-slate-500">Occupation</label>
          <input
            id="occupation"
