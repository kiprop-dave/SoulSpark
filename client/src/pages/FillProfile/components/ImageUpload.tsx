...
export default function ImageUpload({ existingImage, index, setImages }: ImageUploadProps): JSX.Element {
  ...
  return (
    <div className='flex border-2 border-slate-500 border-dashed w-full h-36 rounded relative'>
      {
        !validpreviewUrl ? (
          <form className="w-8 h-8 absolute bottom-0 right-0">
          ...
          <div className="w-full h-full relative" onMouseEnter={() => setDeleteImage(true)} onMouseLeave={() => setDeleteImage(false)}>
            <img
              src={preview}
              alt="profile"
