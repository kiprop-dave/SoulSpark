...
export default function LoginPage() {
  ...
  //TODO: Allow refresh to the same location
  if (user && user.filledProfile) {
  ...
  const onSubmit: SubmitHandler<Credentials> = async (data) => {
    try {
      if (pageVariant === 'login') {
        ...
        if (res.status === 'success') {
          setUserContext(res.user);
          await navigate({ to: '/edit-profile', from: '/' });
        } else {
          setErrorMessage(res.error);
        }
      }
  ...
  return (
    <div className="w-full h-full flex items-center justify-center bg-slate-100 dark:bg-neutral-950">
      <div className="flex flex-col items-center pt-2 w-full h-full sm:w-[30%] sm:h-[95%] bg-white dark:bg-neutral-800 rounded shadow shadow-gray-300 dark:shadow-black overflow-y-scroll no-scrollbar">
        <img src={logo} alt="logo" className="h-20 w-20" />
        <div className="flex flex-col items-center w-full mt-5">
          <h3 className="font-bold text-lg mb-8 dark:text-white">
            {pageVariant === 'login' ? 'Sign in to your account' : 'Create an account'}
          </h3>
          {errorMessage.length > 0 && (
            <p className="text-red-500 text-sm font-semibold" onBlur={() => setErrorMessage('')}>
              {errorMessage}
            </p>
          )}
          <form className="flex flex-col items-center w-full" onSubmit={handleSubmit(onSubmit)}>
            <div className="flex flex-col items-start w-5/6">
              <label htmlFor="email" className="text-sm font-bold dark:text-white">
                Email
              </label>
              {errors.email && (
            ...
            <div className="flex flex-col items-start w-5/6 mt-5 relative">
              <label htmlFor="password" className="text-sm font-bold dark:text-white">
                Password
              </label>
              {errors.password && (
              ...
              <Button
                type="button"
                className="bg-inherit hover:bg-inherit absolute right-0 bottom-0 flex items-center justify-center cursor-text"
              >
                {showPassword === false ? (
                  <AiFillEyeInvisible
          ...
          <div className="mt-5 w-5/6">
            <div className="flex items-center">
              ...
              <p className="mx-2 dark:text-white">or continue with</p>
              <div className="flex-1 h-px bg-gray-500"></div>
            </div>
            <div className="flex justify-center mt-5 w-full">
              <div className="h-14 w-14 border border-gray-200 flex items-center justify-center rounded mx-5 cursor-pointer bg-slate-100">
