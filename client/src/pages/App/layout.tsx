...
export function AppPageLayout({ children }: { children: React.ReactNode }): JSX.Element {
  ...
  return (
    <div className='w-full h-full flex flex-col p-2 justify-between md:flex md:flex-row md:justify-start'>
      <header className="md:hidden h-10 flex items-center">
        <img src={logo} alt="logo" className="w-8 h-8 mr-2" />
        <h1 className="text-2xl font-bold text-red-500">SoulSpark</h1>
      </header>
      <aside className='hidden md:block w-1/4 h-full p-1 border-r border-r-slate-300'>
        <SideBarHeader personalInfo={userProfile?.personalInfo} />
        <SidebarBody />
      </aside>
      <main className='bg-slate-100 w-full md:w-3/4 overflow-scroll no-scrollbar'>
        {children}
      </main>
