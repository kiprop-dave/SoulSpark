...
export function ImagesCarousel({ images }: ImagesCarouselProps): JSX.Element {
  const { currentIndex, nextImage, previousImage, atStart, atEnd } = useImagesCarousel(images.length);
  const [hovered, setHovered] = useState(false);

  const currentImage = images[currentIndex];

  return (
    <div className="w-full h-full p-1 rounded-lg relative overflow-scroll no-scrollbar"
    ...
    >
      <div className="w-[98%] h-[5px] flex items-center justify-between rounded-sm absolute top-2">
        {
          images.map((_, i) => {
            return (
              <div key={i} className={clsx("h-full rounded-sm w-full mx-1", {
      ...
      <div className="w-full h-full flex items-center absolute top-1">
        ...
        >
          <IoChevronBack className={clsx("text-3xl", {
        ...
        <button
          ...
          onClick={nextImage}
        >
          <IoChevronForward className={clsx("text-3xl", {
